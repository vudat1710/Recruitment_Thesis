...
class DuplicateFiltering:
    def __init__(self, num_fields_checked, Y, threshold_tfidf, sim_threshold):
        ...
        for y in Y:
            temp = []
            for i in num_fields_checked:
                temp.append(preprocess(y[i]))
    ...
    def is_match(self, X):
        X_norm = [preprocess(X[i]) for i in range(self.num_fields_checked)]

        size_filtered_candidates = self.size_filtering(X, self.Y_norm)
        candidates = self.position_filtering(X, size_filtered_candidates)
    ...
    def position_filtering(self, X, Y):
        ...
        for i, y in enumerate(Y):
            ...
            for j in range(self.num_fields_checked):
                t = get_prefix_threshold(X[j], y[j])
        ...
        frequent_prefix_y = []
        min_prefix = [99999 for i in range(self.num_fields_checked)]
        for y in prefix_y:
            y_ = []
            for i in range(self.num_fields_checked):
        ...
        for i in range(self.num_fields_checked):
            ...
            for item in frequent_prefix_x[i][:len(frequent_prefix_x[i]) - min_prefix[i] + 1]:
                id_match = inverted_freq_prefix_y[i][item]
                if id_match:
